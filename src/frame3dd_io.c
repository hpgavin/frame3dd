...
void write_input_data (
	...
	float *E, float *G, float *p, float *d, double *L, 
	...
	fprintf(fp,"Jxx     Iyy     Izz       E       G roll  density        L  \n");
	for (i=1; i<= nE; i++) {
		fprintf(fp,"%5d %5d %5d %6.1f %5.1f %5.1f",
					i, J1[i],J2[i], Ax[i], Asy[i], Asz[i] );
		fprintf(fp," %6.1f %7.1f %7.1f %8.1f %7.1f %3.0f %8.2e %14.6f\n",
			Jx[i], Iy[i], Iz[i], E[i], G[i], p[i]*180.0/PI, d[i], L[i] );
	}
